...
def search_knowledge(organization_id, agent_id, query, top_k=5):
    return rag_service.retrieve(organization_id, agent_id, query, top_k=top_k)


def capture_lead(organization_id, agent_id, conversation_id, name=None, email=None,
                  phone=None, requirement=None, location=None, budget=None, is_test=False):
    db = get_db()
    lead_doc = build_lead(
    ...
    if not is_test:
        assignee = assignment_service.auto_assign(organization_id, lead_doc)
        if assignee:
            db.leads.update_one({"_id": result.inserted_id}, {"$set": {"assigned_to": assignee}})
        notification_service.notify_new_lead(organization_id, lead_doc, assignee)
        usage_service.track(organization_id, agent_id, "leads_captured", 1)
    return str(result.inserted_id)


def update_lead(organization_id, lead_id, **fields):
    db = get_db()
    ...
    return get_lead(organization_id, lead_id)
...
def check_availability(organization_id, agent_id, date_str):
...
def book_appointment(organization_id, agent_id, lead_id, start_time_str, kind="appointment",
    ...
    notification_service.notify_appointment_booked(organization_id, doc)
    return {"success": True, "appointment_id": str(result.inserted_id), "start_time": start_time_str}


def request_site_visit(organization_id, agent_id, lead_id, address, start_time_str, notes=None):
    return book_appointment(organization_id, agent_id, lead_id, start_time_str,
                             kind="site_visit", address=address, notes=notes)


def send_notification(organization_id, title, body, link=None):
    notification_service.notify(organization_id, title, body, ntype="general", link=link)
    return {"success": True}


def transfer_to_human(organization_id, conversation_id, reason=""):
    db = get_db()
